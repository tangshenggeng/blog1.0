...
<body>
		...
		<div class="container">
			<nav class="blog-nav">
				<a class="blog-nav-item" href="${APP_PATH}/toWelcome">Home</a> <a
					class="blog-nav-item active" href="#">学习笔记</a> <a
					class="blog-nav-item" href="${APP_PATH }/toFantasy">虚妄之言</a> <a
					class="blog-nav-item" href="${APP_PATH }/aboutAuthor">关于作者</a>
			</nav>
		</div>
	</div>
	<!-- HEADER-END -->
	<div class="container">
		...
		<div class="blog-header">
			...
			<p class="lead blog-description">学习笔记心得</p>
		</div>
...
<script type="text/javascript">
	$(function() {
		//查询所有一级分类下的文章
		$.ajax({
			url : "${APP_PATH}/getAllBlogOfStudy/1",
			type : "GET",
			success : function(result) {
				//展示页面的博客
				build_blogs(result);
				//分页处理
				build_page_ofStu(result);
	...
	//右侧导航栏
	function build_sc(result) {
		//$("#study_sc").empty();
		//查询右侧所有二级分类
		$.ajax({
			url : "${APP_PATH}/getSC/1",
			type : "GET",
			success : function(result) {
	...
	//学习笔记页面下的分页
	function build_page_ofStu(result) {
		$("#blogPage").empty();
		...
		prePageA.click(function() {
			toPageOfStudy(result.extend.pageInfo.pageNum - 1);
		});
	...
	//学习笔记页面的分页跳转
	function toPageOfStudy(pn) {
		$("#studyBlog").empty();
		$.ajax({
			url : "${APP_PATH}/getAllBlogOfStudy/" + pn,
			type : "GET",
			success : function(result) {
				build_blogs(result);
			}
